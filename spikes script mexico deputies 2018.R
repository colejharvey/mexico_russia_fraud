...
keepvars <- c("TOTAL_VOTOS_CALCULADOS", "PRI", "LISTA_NOMINAL_CASILLA") 
...
for(i in 1:max(electoral2$ID_ESTADO)){
  ...
  fraud.estimates[i,1] <- as.character(sub.reg$NOMBRE_ESTADO)[i]
  ...
  filename <- paste("C:/Users/Cole/Documents/Research projects/mexico_russia_fraud/spike_plots/Mexico2018deps/", as.character(fraud.estimates[i,1]),
                    ".png", sep="")
...
write.csv(fraud.estimates, "~/Research projects/mexico_russia_fraud/Mexico/fraud_estimates_mexico_2018_deps_pri.csv")
...
keepvars <- c("TOTAL_VOTOS_CALCULADOS", "PAN", "LISTA_NOMINAL_CASILLA") 
...
for(i in 1:max(electoral2$ID_ESTADO)){
  ...
  fraud.estimates[i,4] <- nrow(sub.reg)
  fraud.estimates[i,5] <- ((fraud.estimates[i,2] / 100) * nrow(sub.reg.spikes)) + fraud.estimates[i,3] 
  filename <- paste("C:/Users/Cole/Documents/Research projects/mexico_russia_fraud/spike_plots/Mexico2018deps/PAN/", as.character(fraud.estimates[i,1]),
                    ".png", sep="")
  png(filename = filename)
  plot.out(out)
  dev.off()
...
write.csv(fraud.estimates, "~/Research projects/mexico_russia_fraud/Mexico/fraud_estimates_mexico_2018_deps_pan.csv")
