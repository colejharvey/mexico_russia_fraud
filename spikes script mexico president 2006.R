...
electoral <- read.csv("C:/Users/Cole/Documents/Grad school/Dissertation/Mexico elections data/mexico 2006 presidential csv.csv")
...
keepvars <- c("total_votes", "PRI", "lista_nominal") 
...
for(i in 1:max(electoral2$id_estado)){
  ...
  fraud.estimates[i,1] <- as.character(sub.reg$id_estado)[i]
  ...
  filename <- paste("C:/Users/Cole/Documents/Research projects/mexico_russia_fraud/spike_plots/Mexico2006pres/", as.character(fraud.estimates[i,1]),
                    ".png", sep="")
...
write.csv(fraud.estimates, "~/Research projects/mexico_russia_fraud/Mexico/fraud_estimates_mexico_2006_pres_pri.csv")
...
keepvars <- c("total_votes", "PAN", "lista_nominal") 
...
for(i in 1:max(electoral2$id_estado)){
  ...
  fraud.estimates[i,4] <- nrow(sub.reg)
  fraud.estimates[i,5] <- ((fraud.estimates[i,2] / 100) * nrow(sub.reg.spikes)) + fraud.estimates[i,3] 
  filename <- paste("C:/Users/Cole/Documents/Research projects/mexico_russia_fraud/spike_plots/Mexico2006pres/PAN/", as.character(fraud.estimates[i,1]),
                    ".png", sep="")
  png(filename = filename)
  plot.out(out)
  dev.off()
...
write.csv(fraud.estimates, "~/Research projects/mexico_russia_fraud/Mexico/fraud_estimates_mexico_2006_pres_pan.csv")
