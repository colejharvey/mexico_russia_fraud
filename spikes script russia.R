...
for(i in 1:max(electoral$temp.id)){
  ...
  if(nrow(sub.reg.spikes) < 100) next #skips empty / small datasets
  #sub.reg.spikes[,1] <- ifelse(sub.reg.spikes$t > sub.reg.spikes$N, 
  #                    sub.reg.spikes$N, sub.reg.spikes$t) #This sets any >100% turnout to 100% so code works
  sub.reg.spikes <- subset(sub.reg.spikes, sub.reg.spikes$t <= sub.reg.spikes$N)
  sub.reg.spikes <- subset(sub.reg.spikes, sub.reg.spikes$t > 0) #Drops obs with 0 votes cast to make code work
  sub.reg.spikes <- subset(sub.reg.spikes, sub.reg.spikes$v > 0) #Drops obs with 0 votes for party to make code work
  sub.reg.spikes <- subset(sub.reg.spikes, sub.reg.spikes$t > sub.reg.spikes$v) #Drops obs with more votes for party than turnout
  set.seed(1274)
  out <- spikes(sub.reg.spikes)  #Runs the spikes program
  fraud.estimates[i,1] <- as.numeric(summary(out)[1])
  fraud.estimates[i,2] <- nrow(sub.reg) - nrow(sub.reg.spikes) #Records dropped observations as suspicious
  fraud.estimates[i,3] <- nrow(sub.reg)
  fraud.estimates[i,4] <- ((fraud.estimates[i,1] / 100) * nrow(sub.reg.spikes)) + fraud.estimates[i,2] 
  cat("Region estimate complete for region ", i, "Estimate: ", fraud.estimates[i])
  alarm()
